...
select * from empleado where cargo like 'secretari_';
...
select nombre, (salario+comision+500) as 'total a pagar' from empleado order by nombre ASC;
...
select nombre,salario,comision from empleado where comision <= salario/100*30;
select * from empleado where !(nombre like '%ma%');
select nombre_depto from departamento where nombre_depto != 'Ventas' and nombre_depto != 'Investigación' and nombre_depto != 'Mantenimiento';
...
select sum(e.salario),d.nombre_depto from empleado e, departamento d group by d.nombre_depto;
SELECT d.nombre_depto, COUNT(e.id_depto)
FROM departamento d, empleado e
WHERE d.id_depto = e.id_depto
GROUP BY d.nombre_depto
HAVING COUNT(d.id_depto) > 3;
/* no puedo hacer el 25 aaaaaaaaaaaaaaaaa*/
SELECT d.nombre_depto, COUNT(e.id_depto)
FROM departamento d, empleado e
WHERE d.id_depto = e.id_depto
GROUP BY d.nombre_depto
HAVING COUNT(d.id_depto) = 0;
SELECT nombre, salario FROM empleado WHERE salario>(SELECT AVG(salario) FROM empleado) order by id_depto ASC;
